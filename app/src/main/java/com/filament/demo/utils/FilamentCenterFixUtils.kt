...
class FilamentCenterFixUtils(
    ...
    /**
     * 核心算法修复统一调度函数：
     * 模型真正的矩阵变换公式：矩阵 = 世界平移 * 旋转 * 缩放 * 抵消中心点的负平移
     */
    private fun applyTransform() {
        ...
        // 应用缩放 (保持 transformToUnitCube 之后的比例)
        Matrix.scaleM(finalMatrix, 0, scaleFactor, scaleFactor, scaleFactor)

        // 核心纠偏：先让模型减去几何中心的偏移值，确保轴心绝对贴合原点
    ...
    fun initModelPosition(distanceFactor: Float, offsetY: Float = 0f) {
        ...
        // 先让 Filament 将其缩放到单位立方体
        modelViewer.transformToUnitCube()

        // 读取此时 transformToUnitCube 赋予它的基本缩放比例
